...
export function getTodos(userId, pageNumber) {
  return new Todo().query({
  ...
  }).orderBy('updated_at','DESC').fetchPage({
    withRelated: ['tags'],
    pageSize: 10,
    page: pageNumber
  }).then((todos) => {
    ...
    return {
      ...
      metadata: {
        ...
        nextPage: todos.pagination.pageCount - todos.pagination.page <= 0 ? null : todos.pagination.page + 1,
        prevPage: todos.pagination.page <= 1 ? null : todos.pagination.page - 1,
      }
    };
  });
}
...
export function getTodoForUser(userId, todoId) {
  ...
  }).fetchAll({withRelated:['tags']}).then((todo) => {
